...
public static class Linker
{
        ...
        BindingFlags bindings, IDictionary<Type, object> parents)
    {
        ...
        foreach (var property in entityType.GetProperties(bindings))
        {
            ...
            if (!TryLinkDecoderAsObject(decoder, objectDescriptor, bindings, property.PropertyType, property.Name,
                    setter, parents))
                return false;
        }
        ...
        foreach (var field in entityType.GetFields(bindings))
        {
            ...
            if (!TryLinkDecoderAsObject(decoder, objectDescriptor, bindings, field.FieldType, field.Name, setter,
                    parents))
                return false;
        }
        ...
        BindingFlags bindings, IDictionary<Type, object> parents)
    {
        ...
        foreach (var property in entityType.GetProperties(bindings))
        {
            ...
            if (!TryLinkEncoderAsObject(encoder, objectDescriptor, bindings, property.PropertyType, property.Name,
                    getter, parents))
                return false;
        }
        ...
        foreach (var field in entityType.GetFields(bindings))
        {
            ...
            if (!TryLinkEncoderAsObject(encoder, objectDescriptor, bindings, field.FieldType, field.Name, getter,
                    parents))
                return false;
        }
        ...
        IDictionary<Type, object> parents)
    {
        ...
        var itemEncoder = MethodResolver
            .Create<Func<EncoderSchema<TNative, TEntity>, IEncoderDescriptor<TNative, object>,
                EncoderSchema<TNative, object>>>(
                (e, o) => e.TurnInto(o))
            .SetGenericArguments(type)
    ...
    private static bool TryLinkEncoderAsValueNullable<TNative, TEntity>(EncoderSchema<TNative, TEntity?> encoder)
